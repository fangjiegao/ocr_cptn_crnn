...
def bbox_overlaps(
        ...
        np.ndarray[DTYPE_t, ndim=2] query_boxes):# boxes:候选anchors, query_boxes:标签boxes
    """
    Parameters
    ----------
    ...
    """
    cdef unsigned int N = boxes.shape[0]  # (1900, 9, 4) N = 1900*9
    cdef unsigned int K = query_boxes.shape[0]  # k * 5
    cdef np.ndarray[DTYPE_t, ndim=2] overlaps = np.zeros((N, K), dtype=DTYPE)  # N * K的矩阵
