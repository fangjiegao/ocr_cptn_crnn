...
from ctpn.bbox import bbox_overlaps
from ctpn.bbox import bbox_transform
from ctpn.rpn_msr.config import Config as cfg
from ctpn.rpn_msr.generate_anchors import generate_anchors

DEBUG = False


# rpn_cls_score:分类器,bbox真实的bbox,im_info:图片的shape
def anchor_target_layer(rpn_cls_score, gt_boxes, im_info, _feat_stride=[16, ], anchor_scales=[16, ]):
    ...
    # 至此，anchor准备好了
    # --------------------------------------------------------------
    # label: 1 is positive, 0 is negative, -1 is dont care (A)
    ...
    overlaps = bbox_overlaps(
        np.ascontiguousarray(anchors, dtype=np.float),  # anchors:(1900*9, 4)
        np.ascontiguousarray(gt_boxes, dtype=np.float))  # 假设anchors有x个，gt_boxes有y个，返回的是一个（x,y）的数组
    # 存放每一个anchor和每一个gtbox之间的overlap:(1900*9,21)
    ...
    # 内部权重，前景就给1，其他是0,为0的权重的意思就是后续的优化中不参与优化
    ...
    # labels
    labels = labels.reshape((1, height, width, A))  # reshap一下label,A=9
    rpn_labels = labels
